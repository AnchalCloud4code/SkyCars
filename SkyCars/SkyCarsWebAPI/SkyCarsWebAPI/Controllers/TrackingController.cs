...
namespace SkyCarsWebAPI.Controllers
{
    ...
    public class TrackingController : BaseController
    {
        ...
        {
            var DocList = await _TrackingService.GetAllAsync(objGrid);
        ...
        public async Task<ApiResponse> Get(int id)
        {
            ...
            var data = await _TrackingService.GetByIdAsync(id);
        ...
        [HttpPost]
        public async Task<ApiResponse> Post(TrackingBookingModel model)
        {
            if (await _TrackingService.IsNameExist(model.LocationName, model.Id))
                return ApiResponseHelper.GenerateResponse(ApiStatusCode.Status400BadRequest, "User name already exists.");

            if (model.Id == 0)
            {
                await _TrackingService.InsertAsync(model.MapTo<Tracking>(), CurrentUserId, CurrentUserName);
            }
            else
            {
                await _TrackingService.UpdateAsync(model.MapTo<Tracking>(), CurrentUserId, CurrentUserName);
            }
        ...
        public async Task<ApiResponse> Delete(IList<int> Ids)
        {
            ...
            var obj = await _TrackingService.GetByIdsAsync(Ids).ConfigureAwait(false);
            if (obj == null)
            ...
            await _TrackingService.UpdateAsync(obj, CurrentUserId, CurrentUserName);
