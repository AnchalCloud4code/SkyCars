...
namespace SkyCarsWebAPI.Controllers
{
    ...
    [ApiController]
    [ApiVersion("1")]
    public class refundBookingController : BaseController
    {
        #region Fields

        private readonly IRefundService _RefundRepository;

        #endregion

        #region Ctor

        public refundBookingController(IRefundService RefundService)
        {
            _RefundRepository = RefundService;
        }
        ...
        {
            var DocList = await _RefundRepository.GetAllAsync(objGrid);
        ...
        [HttpPost]
        public async Task<ApiResponse> Post(Refund model)
        {
            //if (await _RefundRepository.IsNameExist(model.Name, model.Id))
            //    return ApiResponseHelper.GenerateResponse(ApiStatusCode.Status400BadRequest, "User name already exists.");
            if(model.Id == 0)
            {
                await _RefundRepository.InsertAsync(model.MapTo<Refund>(), CurrentUserId, CurrentUserName);
            }
            else
            {
                await _RefundRepository.UpdateAsync(model.MapTo<Refund>(), CurrentUserId, CurrentUserName);
            }
        ...
        public async Task<ApiResponse> Delete(IList<int> Ids)
        {
            ...
            var obj = await _RefundRepository.GetByIdsAsync(Ids).ConfigureAwait(false);
            if (obj == null)
            ...
            await _RefundRepository.UpdateAsync(obj, CurrentUserId, CurrentUserName);
